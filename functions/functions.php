...
//                                 <a type='button' class='btn btn-secondary submit fa fa-bars m-1'>More</a>
//                             </div>
//                         </div>
//                     </div>";
//             }
...
function getUniqueCategories()
{
  ...
  if (isset($_GET['category'])) {
    ...
    <div class='card'>
      ...
      <p class='card-text'><b>&#8377;$product_price/-</b></p>
        <a type='button' class='btn  fa fa-shopping-cart m-1 submit'></a>
        <a type='button' href='product_details?product_id=$product_id' class='btn submit fa fa-bars m-1 '  > More</a>
    </div>
    </div>
  </div>";
    }
...
function searchProducts()
{
  ...
  if (!isset($_GET["search_product"])) {
    ...
    $num_of_rows = mysqli_num_rows($result_query);
    if ($num_of_rows == 0) {
      echo "<h1 class='text-center w-75 text-danger'>No Resullt Matched, No Product found for this category</h2>";
    }
    if ($result_query) {
      while ($row = mysqli_fetch_assoc($result_query)) {
        $product_id = $row['product_id'];
